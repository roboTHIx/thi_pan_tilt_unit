...
void setPositionCallback(const std_msgs::Float64MultiArray::ConstPtr & msg)
{
  uint8_t dxl_error       = 0;
  int dxl_comm_result     = COMM_TX_FAIL;
  int dxl_addparam_result = false;
  ...
  uint32_t position1 = (unsigned int)msg->data[0]; // Convert int32 -> uint32
  ROS_ERROR_STREAM("Pos1: " << position1); 
  ...
  uint32_t position2 = (unsigned int)msg->data[1]; // Convert int32 -> uint32

  ROS_ERROR_STREAM("Pos2: " << position2); 
  ...
  dxl_addparam_result = groupSyncWrite.addParam(1, param_goal_position1);
  if (dxl_addparam_result != true) 
  {
    ROS_ERROR( "Failed to addparam to groupSyncWrite for Dynamixel ID %d", 1);
  }

  dxl_addparam_result = groupSyncWrite.addParam(2, param_goal_position2);
  if (dxl_addparam_result != true) 
  {
    ROS_ERROR( "Failed to addparam to groupSyncWrite for Dynamixel ID %d", 2);
  }
...
int main(int argc, char ** argv)
{
  ...
  // switch on LEDs
  dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, DXL1_ID, ADDR_LED, 1, &dxl_error);
  dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, DXL2_ID, ADDR_LED, 1, &dxl_error);


  // set the position mode for all drives
  dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, DXL1_ID, ADDR_DRIVE_MODE, 3 &dxl_error);
  if (dxl_comm_result != COMM_SUCCESS) 
  {
    ROS_ERROR("Failed to set position mode %d", DXL1_ID);
    return -1;
  }
  
  dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, DXL2_ID, ADDR_DRIVE_MODE, 3, &dxl_error);
  if (dxl_comm_result != COMM_SUCCESS) 
  {
    ROS_ERROR("Failed to set position mode %d", DXL2_ID);
    return -1;
  }
  ...
  ros::Subscriber set_position_sub = nh.subscribe("pan_tilt/position", 10, setPositionCallback); 
  ros::spin();

  // switch off leds
  dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, DXL1_ID, ADDR_LED, 0, &dxl_error);
  dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, DXL2_ID, ADDR_LED, 0, &dxl_error);
